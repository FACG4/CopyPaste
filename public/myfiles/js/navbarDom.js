const option=Array.prototype.slice.call(document.querySelectorAll('.nav-color-select'))
const inputs=Array.prototype.slice.call(document.querySelectorAll('.nav-color-input'))
let color=''
window.onload=()=>{
...
option.forEach(element=>{
  ...
  element.addEventListener('input',e=>{
    ...
    document.querySelector('#'+(element.options[element.selectedIndex].value)).style.display='inline-block';
     colorType= document.querySelector('#'+(element.options[element.selectedIndex].value)).id.split('nav-color-input-')[1].split('-')[0]
